...
#Add anticipation; outcome changes when treatment period approaches;
#AE <- c(0.2, 0.5) 
df <- df %>% 
  ...
  mutate(Y0 = beta[1] + x1*beta[2] + x2*beta[3] + x4*beta[4] + 1*t + e + 
           ifelse(G_star<10000, cos(t)+3*t, 0) ) %>% #Give the different time trend for those who is (will be) treated; make supersurvivor and others different
  #mutate(Y0 = beta[1] + x1*beta[2] + x2*beta[3] + x4*beta[4] + 1*t + e +
  #         ifelse(G_star<10000, AE[1]*(tau==-2) + AE[2]*(tau==-1) , 0)) %>%
  mutate(Y1 = Y0 + 1) %>%
...
write_csv(df, "analysis/code/testDGP.R")
