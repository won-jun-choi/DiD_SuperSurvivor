...
# supersurvivor logit
gamma <- c(1, 2, 0.5)  # supersurvivor logit parameters
df <- df %>% 
  group_by(unit) %>%
  mutate(p_uncured = exp(gamma[1] + gamma[2]*x1 + gamma[3]*x2) / (1 + exp(gamma[1] + gamma[2]*x1 + gamma[3]*x2))) %>%
  mutate(p_cured = 1 - p_uncured) %>%
  mutate(C_tilde = ifelse(runif(1)<p_uncured,0,1), # supersurvivor indicator
         C_tilde = ifelse(runif(1)<p_cured,1,0)) %>%
  ungroup()
...
# survival function
lambda <- c(1, 2, 3)  # parameters for the survival function
df <- df %>% 
  group_by(unit) %>%
  mutate(u = runif(1),
