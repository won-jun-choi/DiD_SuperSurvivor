...
#'
#'
DiD_supersurvivor_NoControl <- function(data,
                              unit_variable,
                              time_variable,
                              group_variable,
                              outcome_variable,
  ...
  for (i in 1:nrow(results)) {
    ...
    m0_reweight <- lm('dY ~ 1', data=C_group, weights=phat)
    Em0_reweight <- predict(m0_reweight, newdata=C_group) %>% mean()
    Em0_reweight2 <- predict(m0_reweight, newdata=T_group) %>% mean()
    Em0_reweight3 <- sum(C_group$dY * C_group$phat) / sum(C_group$phat)
    results[i, 'ATT_SS_noX'] <- Em1 - Em0_reweight
...
if (sys.nframe() == 0) {
  ...
  res <- DiD_supersurvivor_NoControl(data = df,
                           unit_variable = 'unit',
                           time_variable = 't',
                           group_variable = 'G',
